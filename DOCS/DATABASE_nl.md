...
#### Deduplicatie via vendor_id en old_id
`production`, `event` en `event_price` hebben een `vendor_id` — het unieke ID uit de VIERNULVIER ticketing-API — dat tijdens de automatische synchronisatie gebruikt wordt om duplicaten te vermijden. Daarnaast hebben `production`, `event`, `event_price`, `hall`, `image`, `crop` en `tag` een `old_id` (UNIQUE). `old_id` verwijst naar de primary key van hetzelfde record in het oude archiefsysteem, zodat de historische CSV-import (2006–2018) opnieuw uitgevoerd of gereconcilieerd kan worden zonder duplicaten te produceren.
