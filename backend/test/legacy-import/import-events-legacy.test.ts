...
describe("makeLegacyKey (missing field fallbacks)", () => {
  it("uses empty string for missing fields (ln 125 ?? '' fallbacks)", () => {
    ...
    expect(key).toHaveLength(40); // SHA-1 hex
  });
});
...
describe("importEventsLegacy", () => {
  ...
  it("logs Row limit line when args.limit is set (ln 320)", async () => {
    ...
    const query = vi.fn().mockImplementation((sql: string) => {
      const dedupe = handleProductionDedupeQueries(sql, [{ id: 1 }], false);
      if (dedupe) return Promise.resolve(dedupe);
      if (sql.includes("to_regclass")) return Promise.resolve({ rows: [{ exists: "legacy_production_import_map" }] });
      if (sql.includes("FROM legacy_production_import_map")) {
        return Promise.resolve({ rows: [{ legacy_id: "p1", production_id: 1, created_new_production: false }] });
      }
      if (sql.includes("FROM hall") && sql.includes("SELECT id")) return Promise.resolve({ rows: [] });
      return Promise.resolve({ rows: [] });
    });
  ...
  it("skips productions CSV rows with no id (ln 186-187) and uses titel/ondertitel fallbacks (ln 188)", async () => {
    ...
    // Should complete without error — the no-id row is skipped, the valid row proceeds
    await importEventsLegacy(makeClient(query), {
      filePath: csvPath,
      dryRun: true,
      limit: null,
  ...
  it("skips dedupe loop when production meta title is empty (ln 370)", async () => {
    ...
    const prodCsv = writeProductionsCsv(dir, ["Titel,ID,Ondertitel", ",p1,"]);
    fs.writeFileSync(csvPath, "Starttime,Endtime,Hall,Production\n2024-01-01 10:00:00,,Main,p1\n", "utf8");
  ...
  it("write mode: second event with same hall hits hallCache (ln 224), limit stops reading at 2 rows (ln 345)", async () => {
    ...
    // 3 rows, different start times so different keys, same hall — limit=2 so third is never read
    fs.writeFileSync(
      csvPath,
      [
        "Starttime,Endtime,Hall,Production",
        "2024-01-01 10:00:00,,SameHall,p1",
        "2024-01-02 10:00:00,,SameHall,p1",
        "2024-01-03 10:00:00,,SameHall,p1",
      ].join("\n") + "\n",
      "utf8",
    );
    ...
    const query = vi.fn().mockImplementation((sql: string) => {
      const dedupe = handleProductionDedupeQueries(sql, [{ id: 1 }], false);
      if (dedupe) return Promise.resolve(dedupe);
      if (sql.includes("CREATE TABLE IF NOT EXISTS legacy_event_import_map")) return Promise.resolve({ rows: [] });
      if (sql.includes("to_regclass")) return Promise.resolve({ rows: [{ exists: "legacy_production_import_map" }] });
      if (sql.includes("FROM legacy_production_import_map")) {
        return Promise.resolve({ rows: [{ legacy_id: "p1", production_id: 1, created_new_production: false }] });
      }
      if (sql.includes("SELECT id, name FROM hall")) return Promise.resolve({ rows: [] });
      if (sql.includes("SELECT legacy_key") && sql.includes("legacy_event_import_map")) return Promise.resolve({ rows: [] });
      if (sql.trimStart().startsWith("BEGIN")) return Promise.resolve({ rows: [] });
      if (sql.trimStart().startsWith("COMMIT")) return Promise.resolve({ rows: [] });
      if (sql.trimStart().startsWith("ROLLBACK")) return Promise.resolve({ rows: [] });
      if (sql.includes("jsonb_each_text(name)") && sql.includes("FROM hall")) return Promise.resolve({ rows: [] });
      if (sql.includes("INSERT INTO hall")) return Promise.resolve({ rows: [{ id: 5 }] });
      if (sql.includes("INSERT INTO event")) return Promise.resolve({ rows: [{ id: 99 }] });
      if (sql.includes("INSERT INTO legacy_event_import_map")) return Promise.resolve({ rows: [] });
      return Promise.resolve({ rows: [] });
    });
  ...
  it("dry-run: hall not found in DB takes the dryRun early-return path (ln 239)", async () => {
    ...
    fs.writeFileSync(csvPath, "Starttime,Endtime,Hall,Production\n2024-03-01 10:00:00,,BrandNewHall,p1\n", "utf8");
    ...
    expect(query.mock.calls.some((c) => String(c[0]).includes("INSERT INTO hall"))).toBe(false);
    fs.rmSync(dir, { recursive: true });
  });
});