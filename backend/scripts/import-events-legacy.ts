...
function parseHall(value: string): ParsedHall | null {
...
function makeLegacyKey(row: Record<string, string>): string {
  const payload = `${row["starttime"] ?? ""}|${row["endtime"] ?? ""}|${row["hall"] ?? ""}|${row["production"] ?? ""}`;
  return crypto.createHash("sha1").update(payload).digest("hex");
}

function hallKey(name: string): string {
