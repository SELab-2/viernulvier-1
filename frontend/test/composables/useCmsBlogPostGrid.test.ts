...
describe("useCmsBlogPostGrid", () => {
  it("declares the blogpost column definitions", () => {
    const { columnDefs } = useCmsBlogPostGrid({ isDark: ref(false), t: (key) => key });
  ...
  it("builds translated column options", () => {
    const { gridColumnOptions } = useCmsBlogPostGrid({ isDark: ref(false), t: (key) => key });
  ...
  it("exposes a pinned selection column definition", () => {
    const { selectionColumnDef } = useCmsBlogPostGrid({ isDark: ref(false), t: (key) => key });
  ...
  it("provides a non-editable defaultColDef with floating filter", () => {
    const { defaultColDef } = useCmsBlogPostGrid({ isDark: ref(false), t: (key) => key });
  ...
  it("renders the id cell as a link to the blog post", () => {
    ...
    expect(html).toContain('href="/blog/post/42"');
  });
});